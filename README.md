...
| `/*`       | `api/proxy.js`   | Пробрасывает на `api.telegram.org`, отклоняет чужие токены (403)   |

## Переменные окружения

| Переменная       | Описание                                                                     |
|------------------|------------------------------------------------------------------------------|
| `MTRL_WEBHOOK_URL` | URL для relay входящих webhook. Значение: `https://mtrl.io/api/tg-auth`   |
| `BOT_TOKEN`      | Токен боевого бота. Запросы с другим токеном получат 403                     |
| `BOT_TOKEN_DEV`  | Токен дев-бота (`@mtrl_test_bot`). Опционально                               |
